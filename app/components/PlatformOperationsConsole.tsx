  ...
  return (
    <section className="platform-console" id="admin-platform" aria-labelledby="platform-console-title">
      ...
      <div className="platform-console__grid">
          ...
          <ul className="platform-console__list">
            {providers.map((probe) => {
              ...
              return <li key={provider}>
                <div><strong>{provider}</strong><span data-state={probe.status || "unknown"}>{probe.status || "미점검"}</span><small>{probe.detail || "최근 진단 기록이 없습니다."}{latency !== undefined ? ` · ${latency}ms` : ""}</small></div>
        ...
        <section>
          <div className="platform-console__section-head"><h3>RAG 파이프라인</h3><span>원격 진단</span></div>
          <ul className="platform-console__list">
            {(pipeline.components ?? []).map((component) => {
              const result = pipelineProbes[component.id];
              return <li key={component.id}>
                <div><strong>{component.name}</strong><span data-state={result?.status || "unknown"}>{result?.status || "미점검"}</span><small>{result?.detail || "연결 진단을 실행하면 결과가 표시됩니다."}{result?.latencyMs !== undefined ? ` · ${result.latencyMs}ms` : ""}</small></div>
