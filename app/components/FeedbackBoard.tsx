...
const filters: Array<{ value: "all" | FeedbackCategory; label: string }> = [
  ...
  { value: "notice", label: text.notice },
  { value: "feature", label: text.feature },
  { value: "bug", label: text.bug },
...
export function FeedbackBoard() {
  ...
  const toggleLike = async (post: FeedbackPost) => {
    ...
    try {
      ...
      setError(likeError instanceof Error ? likeError.message : "\uC88B\uc544\uc694 \uCC98\uB9AC\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.");
    } finally {
    ...
    <div className="view-stack feedback-board">
      ...
      <div className="feedback-layout">
            ...
            {visiblePosts.map((post) => {
              ...
              return <article className={`feedback-post ${isOpen ? "is-open" : ""}`} key={post.id}>
                <button className="feedback-post-summary" type="button" aria-expanded={isOpen} onClick={() => setOpenPostId(isOpen ? null : post.id)}>
                  <span className={`feedback-category feedback-category-${post.isNotice ? "notice" : post.category}`}>{post.isNotice ? text.notice : categoryLabels[post.category]}</span>
                  <span className="feedback-post-main"><strong>{post.title}</strong><span className="feedback-author"><span className="feedback-author-avatar" aria-hidden="true">{post.authorName.slice(0, 1)}</span><span><b>{post.authorName}{post.isMine ? ` · ${text.authorMine}` : ""}</b><small>{post.authorDepartment ? `${post.authorDepartment} · ` : ""}{formatDate(post.createdAt)}</small></span></span></span>
                ...
                {isOpen && <div className="feedback-post-content">
                  ...
                  <div className="feedback-comments">
                    ...
                    {post.comments.length > 0 ? <div className="feedback-comment-list">{post.comments.map((comment) => <div className="feedback-comment" key={comment.id}><span className="feedback-author-avatar" aria-hidden="true">{comment.authorName.slice(0, 1)}</span><div><strong>{comment.authorName}{comment.isMine ? ` · ${text.authorMine}` : ""}</strong><small>{comment.authorDepartment ? `${comment.authorDepartment} · ` : ""}{formatDate(comment.createdAt)}</small><p>{comment.content}</p></div></div>)}</div> : <p className="feedback-comment-empty">{"첫 댓글을 남겨 주세요."}</p>}
        ...
        <section className="panel feedback-form-panel" aria-labelledby="feedback-form-title">
          <div><span className="section-kicker">SHARE AN IDEA</span><h2 id="feedback-form-title">{"의견 남기기"}</h2><p className="panel-description">{"개선점과 사용 경험을 공유해 주세요."}</p></div>
          <form className="feedback-form" onSubmit={submit}>
            <label>{"분류"}<select value={draft.category} onChange={(event) => setDraft((current) => ({ ...current, category: event.target.value as FeedbackCategory }))}><option value="feature">{text.feature}</option><option value="bug">{text.bug}</option><option value="question">{text.question}</option><option value="other">{text.other}</option></select></label>
            ...
            <div className="feedback-form-foot"><small>{draft.content.length.toLocaleString("ko-KR")} / 5,000{"자"}</small><button className="button button-primary" type="submit" disabled={saving || !draft.title.trim() || !draft.content.trim()}>{saving ? "등록 중" : "게시글 등록"}</button></div>
          </form>
