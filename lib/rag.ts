...
export function fitRagPrompt(template: string, evidence: string) {
...
export function boundFallbackMessages(messages: GatewayMessage[]) {
...
export async function completeWithRag(input: {
  ...
  if (!search.grounded) {
    ...
    const fallbackMessages = input.contextFileBlock
      ? [{ role: "user" as const, content: `${input.contextFileBlock}\n사용자 질문:\n${latestUserMessage}` }]
      : input.messages;
    const fallbackCompletion = await completeWithGateway(
      boundFallbackMessages(fallbackMessages),
      input.traceId,
      input.providerPolicy,
      reasoningTier,
    );
