...
test("RAG schema preflight covers every runtime-owned table and budget columns", () => {
  for (const table of ["assets", "segments", "index_jobs", "retrieval_traces", "visual_regions", "visual_embeddings", "segments_fts", "ingestion_sources"]) {
    assert.match(source, new RegExp(`${table}: \\[`));
  }
  for (const column of ["deferred_until", "resume_offset", "last_error_code", "visual_search_enabled", "chart_json", "char_start", "char_end"]) {
